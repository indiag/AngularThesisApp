...
angular.module('myApp.controllers', ['ngSanitize','angular-flexslider'])
    ...
    }).controller('MyCtrl9', function ($scope, $http, $sce) {



        $scope.callPartialTen = function () {

            $http.get('./partials/partial10.html').success(function (data, status) {
                $scope.partialTenContent = data;
                $scope.trustedHtml = $sce.trustAsHtml($scope.partialTenContent);
            });

        };

        $scope.callPartialEleven = function () {

            $http.get('./partials/partial11.html').success(function (data, status) {
                $scope.partialElevenContent = data;
                $scope.trustedHtml = $sce.trustAsHtml($scope.partialElevenContent);
            });
        };
